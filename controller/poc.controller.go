...
func (e *Poc) SaveBlogs(w http.ResponseWriter, r *http.Request) {
	...
	for _, blog := range blogs {
		...
		if blog.ID < 0 {
			// Insert TecMember with negative ID
			res, err = e.repo.CreateBlog(r.Context(), &blog)
		} else {
			// Update TecMember with non-negative ID
...
func (e *Poc) SaveTecActivity(w http.ResponseWriter, r *http.Request) {
	...
	var tecTimelines []models.TECTimeline

	err = json.NewDecoder(r.Body).Decode(&tecTimelines)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		logging.Logger.Errorf(err.Error())
		return
	...
	// Iterate over the slice of tecMembers
	for _, tecTimeline := range tecTimelines {
		res, err := e.repo.CreateTecTimeline(r.Context(), &tecTimeline)
		if err != nil {
			respondWithError(w, http.StatusForbidden, err.Error())
			logging.Logger.Errorf(err.Error())
			return
...
func (e *Poc) UpdateDescription(w http.ResponseWriter, r *http.Request) {
	...
	res, err := e.repo.UpdateDescription(r.Context(), req.Description, id)
